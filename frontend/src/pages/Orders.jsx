            ...
            <div className="space-y-6">
              {
                orders.map((order, index) => (
                  <div key={order._id} className="border py-5 px-6 rounded-lg shadow-md flex flex-col gap-2">
                    ...
                    <p className=""><strong>Order Created:</strong> {new Date(order.createdAt).toLocaleDateString()} at {new Date(order.createdAt).toLocaleTimeString()}</p>
                    <p className=""><strong>Products:</strong> {order.productIds.join(', ')}</p>
                  </div>
                ))
              }
            </div>
